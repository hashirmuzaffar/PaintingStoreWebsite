    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            try {
                ...
                CallableStatement cstmt = conn.prepareCall("{? = call DISPLAY_ARTIST_DETAILS (?)}");

                // Declare that the first ? is a return value of type Int
                cstmt.registerOutParameter(1, Types.VARCHAR);

                // We want to raise LESLIE's salary by 20,000
                cstmt.setInt(2, A_no);  // The name argument is the second ?
                ...
                CallableStatement cstmt2 = conn.prepareCall("{? = call ARTIST_REPORT_VIEW (?)}");

                // Declare that the first ? is a return value of type Int
                cstmt2.registerOutParameter(1, Types.VARCHAR);

                // We want to raise LESLIE's salary by 20,000
                cstmt2.setInt(2, A_no);  // The name argument is the second ?
                ...
                out.println("--------------------------------------------------------------------------------------------" + "<br>");
                out.println("Painting No | " + "Painting Title | " + "Theme | " + "Rental Price | " + "Owner no | " + "Owner Name | " + "Owner Tel");
                for (int i = 0; i < str.length; i++) {
