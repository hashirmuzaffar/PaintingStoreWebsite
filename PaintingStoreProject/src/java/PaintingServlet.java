     ...
     */
     protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            int P_no = Integer.parseInt(request.getParameter("P_Number"));
            String P_title = request.getParameter("Title");
            String P_theme = request.getParameter("Theme");
            int P_price = Integer.parseInt(request.getParameter("price"));
            int A_no = Integer.parseInt(request.getParameter("A_Number"));
            int O_no = Integer.parseInt(request.getParameter("O_Number"));
            ...
            try{
               ...
               /** CallableStatement cstmt = myCon.prepareCall("{? = call eligble_to_ReHire (?)}");
                ...
                cstmt.close();
                if(status.equals("K")){**/
                String qry = "BEGIN add_painting(?,?,?,?,?,?); END;";
                PreparedStatement myStmt; 
                myStmt = myCon.prepareStatement(qry);
                myStmt.setInt(1, A_no);
                myStmt.setInt(2, P_no);
                myStmt.setString(3, P_title);
                myStmt.setString(4, P_theme);
                myStmt.setInt(5, P_price);
                myStmt.setInt(6, O_no);
               
                int r = myStmt.executeUpdate(); 
                 myStmt.close();
                if(r ==1){
                out.println(P_title + "<h2 class='heading'> is avaiable for rent</h><br>");
                ...
                {
                out.println("<h2>Looks like a problem has occured while Adding </h>"+ P_title);
                out.println("<a href='index.html'>");
                ...
                /**else{
                out.println("<h2>Looks like a problem has occured while Adding </h>"+ P_title);
                out.println("<h2>Its not been three month since its return</h>"+ P_title);
                out.println("<a href='index.html'>");
                out.println("<button class='button enter Customer'>CANCEL</button>");
                out.println("</a>");
                }**/
